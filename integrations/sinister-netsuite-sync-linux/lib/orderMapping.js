    ...
    const options = (item.options || [])
      ...
      .map((option, index) => {
        ...
        return {
          ...
          taxable: moneyToCents(item.tax) > 0,
          description: [option.attr_prompt, option.opt_prompt].filter(Boolean).join(': ') || optionSku,
          mivaLineId: item.line_id,
        };
      });
...
function validateMivaOrderTotals(order, expandedLines) {
  for (const item of order.items || []) {
    ...
    if (Math.abs(expected - actual) > 1) {
      throw new Error(`Miva item ${item.line_id} does not reconcile within one cent: expected ${expected}, expanded ${actual}`);
    }
  }
  ...
  if (Math.abs(productCents + chargeCents - orderCents) > 1) {
    throw new Error(`Miva order ${order.id || 'unknown'} does not reconcile within one cent`);
  }
...
async function resolveExpandedLines(lines, lookup, overrides = {}) {
  ...
  for (const line of lines) {
    if (overrides[line.sku]) {
      resolved.push({ ...line, itemId: String(overrides[line.sku]) });
      continue;
    }
    let match = null;
...
function assertTotalsMatch(mivaTotal, netsuiteTotal) {
  const difference = Math.abs(moneyToCents(mivaTotal) - moneyToCents(netsuiteTotal));
  if (difference > 1) throw new Error(`NetSuite total ${netsuiteTotal} does not reconcile with Miva total ${mivaTotal}`);
  return true;
}
