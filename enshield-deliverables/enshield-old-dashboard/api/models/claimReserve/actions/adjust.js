...
export const run = async ({ params, record, api, session }) => {
  ...
  const operation = await claimOperationalOperation(api, { operationKey, accountingEntityId: entity.id, operation: "adjust_reserve", actorId, resultRecordId: record.id });
  ...
  record.additionsMinor = Number(record.additionsMinor || 0) + amountMinor;
  record.closingMinor = calculateReserveRollForward(record);
  record.reserveRevision = reserveRevision + 1;
  await save(record);
  await api.internal.claimReserveMovement.create({ accountingEntity: { _link: String(entity.id) }, claimReserve: { _link: String(record.id) }, operationKey, currency: record.currency, movementType: "addition", amountMinor, effectiveAt: new Date().toISOString() });
  await writeAudit(api, { shopId: entity.shopId, accountingEntityId: entity.id, actorEmail: access.operator.email || null, action: "finance.reserve.adjust", entityType: "claimReserve", entityId: record.id, after: { amountMinor, closingMinor: record.closingMinor, currency: record.currency, shadowMode: true } });
