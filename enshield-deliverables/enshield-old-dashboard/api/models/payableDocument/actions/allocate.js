...
export const run = async ({ params, record, api, session }) => {
  ...
  await api.internal.payableAllocation.create({
    accountingEntity: { _link: String(entity.id) },
    payableDocument: { _link: String(record.id) },
  ...
  await writeAudit(api, {
    ...
    ...sanitizedFinanceAudit({ action: "finance.payable.allocate", entityType: "payableDocument", entityId: record.id, status: record.status, amountMinor: allocation.amountMinor, currency: allocation.currency }),
  });
