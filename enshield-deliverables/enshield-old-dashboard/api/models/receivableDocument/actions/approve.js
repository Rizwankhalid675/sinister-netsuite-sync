...
export const run = async ({ record, api, session }) => {
  ...
  await save(record);
  await writeAudit(api, { shopId: entity.shopId, accountingEntityId: entity.id, actorEmail: access.operator.email || null, action: "finance.receivable.approve", entityType: "receivableDocument", entityId: record.id, after: { status: record.status, shadowMode: true } });
